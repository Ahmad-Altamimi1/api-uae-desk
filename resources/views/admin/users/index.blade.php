                    ...
                    <div class="col-md-3">
                        <div class="create-btn pull-right">
                            <a href="{{ route('users.create') }}"
                                class="btn custom-create-btn">{{ __('default.form.add-button') }}</a>
                        </div>
                    </div>
                @endif
            </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                @if (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Super Admin'))
                                    <th class="">{{ __('Created By') }}</th>
                                    <th class="">{{ __('Updated By') }}</th>
                                @endif
                                <th class="">{{ __('default.table.status') }}</th>
                                <th class="">{{ __('user.index.shift') }}</th>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).on('change', '.shift-dropdown', function() {
            var userId = $(this).data('user-id');
            var shiftId = $(this).val();
            let _token = $('meta[name="csrf-token"]').attr('content');

            $.ajax({
                url: `{{ route('users.update_shift') }}`,
                type: 'PUT',
                data: {
                    _token: _token,
                    user_id: userId,
                ...
                error: function(xhr) {
                    toastr.error('An error occurred while updating the shift.');
                }
            });
        });
    </script>
